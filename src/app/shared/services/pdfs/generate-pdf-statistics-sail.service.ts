...
export class GeneratePdfStatisticsSailService {
  ...
  async generatePDFStatistic(sails: any, user: User, fechaIncio: any, fechaFin: any, selectedCategory: Category[], selectedProduct: Product){
    ...
    pdf.add(
      new Txt(`\n ${sails.length} ${sails.length < 2 ? 'Egreso' : 'Egresos'}`).alignment('right').bold().fontSize(10).margin(10).end
    );  
    ...
    pdf.add(
      new Table([
        ...
        // [ 'header 1.1', 'Header 2.1'],
        // [ 'header 1.1', 'Header 2.1'],
      ]).widths(['*','*']).keepWithHeaderRows(1).headerRows(1).end
    )
    ...
    //     ],
    //   ]).keepWithHeaderRows(1).headerRows(1).widths([90,90,80,120,80,110, 50, 60]).bold().fontSize(8).end
    // );

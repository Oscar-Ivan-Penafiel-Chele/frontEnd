...
import { Category, User } from '@models/interfaces';
...
export class CategoryComponent implements OnInit {
  ...
  clearImage(){
    ...
    this.photoSelected = "";
    this.isPhotoEdit = false;
    this.category.category_thumbnail = '';
  }
  ...
  updateData(existImage : boolean){
    ...
    this.categoryService.updateCategory(data, this.category.id_category!)
    .subscribe((response)=>{
        if(response.status == 200 && response.message === "Categoria actualizada con exito"){
            this.getCategories();
            this.hideDialog();
            this.messageService.add({severity:'success', summary: 'Completado', detail: 'La categoria fue actualizada con éxito', life: 3000});
